...
class MultifunctionalMapper {
     ...
     */
    public static final Function<List<IntUnaryOperator>, UnaryOperator<List<Integer>>> MULTIFUNCTIONAL_MAPPER = mappers -> nums -> nums
            .stream()
            .map(n -> mappers
                    .stream()
     ...
     */
    public static final UnaryOperator<List<Integer>> MULT_TWO_AND_THEN_ADD_ONE_TRANSFORMATION = MULTIFUNCTIONAL_MAPPER.apply(Arrays.asList(x -> x * 2, x -> x + 1));
    /**
     * The operator accepts an integer list.
     ...
     * The operator returns transformed integer list.
     */
    public static final UnaryOperator<List<Integer>> SQUARE_AND_THEN_GET_NEXT_EVEN_NUMBER_TRANSFORMATION = MULTIFUNCTIONAL_MAPPER.apply(Arrays.asList(x -> x * x, x -> x % 2 == 0 ? x + 2 : x + 1));
